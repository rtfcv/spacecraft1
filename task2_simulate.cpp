...
int task2::simulate_sym_filtered(int t, Eigen::VectorXd y0){
    ...
    Eigen::VectorXd xhat=y0*0;

    Eigen::VectorXd xbar=y0*0;

    Eigen::VectorXd z_int1=Eigen::VectorXd::Zero(7);

    Eigen::Vector3d v=Eigen::VectorXd::Zero(3);

    _P_diag_result.col(0) << std::sqrt(P_n(0,0)), std::sqrt(P_n(1,1)), std::sqrt(P_n(2,2)), std::sqrt(P_n(3,3)), std::sqrt(P_n(4,4)), std::sqrt(P_n(5,5)), std::sqrt(P_n(6,6));
